...
export function OnboardingPage({ onComplete }: Props) {
  ...
  return <main className="auth-page" style={{ alignItems: "flex-start", paddingTop: 44 }}>
    <section className="auth-card" style={{ maxWidth: 820, width: "100%" }}>
      ...
      {step === 2 && <div>
        <p className="eyebrow">Operation</p><h2>What does this fleet actually do?</h2><p className="subtle">FleetOS will use this profile to drive relevant setup and rules instead of treating every operator the same.</p>
      ...
      {step === 3 && <div>
        <p className="eyebrow">Compliance profile</p><h2>Tell FleetOS which framework applies.</h2><p className="subtle">This does not claim accreditation. It tells FleetOS which evidence, dates and checks your company wants to manage.</p>
      ...
      {step === 4 && <div>
        ...
        <div style={{ display: "grid", gap: 8, color: "#475569", fontSize: 14 }}><span><CheckCircle2 size={16} style={{ verticalAlign: "middle", marginRight: 7 }}/>You become Company Admin.</span><span><CheckCircle2 size={16} style={{ verticalAlign: "middle", marginRight: 7 }}/>No demo company or fake fleet data is created.</span><span><CheckCircle2 size={16} style={{ verticalAlign: "middle", marginRight: 7 }}/>Vehicles, people and evidence are added once and reused across FleetOS.</span></div>
      </div>}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginTop: 24 }}>
        ...
        {step < 4 ? <button type="button" className="primary-button" onClick={next}>Continue <ArrowRight size={17}/></button> : <button type="button" className="primary-button" disabled={busy} onClick={() => void submit()}>{busy ? "Creating workspace…" : <>Open my FleetOS <ArrowRight size={17}/></>}</button>}
      </div>
    </section>
  </main>;
}
