...
export function PersonalPage() {
  ...
  if (saved) return (
    <section className="page"><div className="empty-state panel"><div className="company-dot" style={{ margin: "0 auto 16px" }}><Check /></div><h1>Person added</h1><p>{form.firstName} {form.lastName} is now in the selected company people records.</p><p className="subtle">{form.inviteAccount ? "An account invitation has been sent or the existing account has been linked." : "No app account was created."}</p><button className="primary-button" onClick={() => window.location.reload()}>Add another person</button></div></section>
  );

  return (
    <section className="page">
      <div className="page-heading"><div><p className="eyebrow">People & access</p><h1>Personal</h1><p className="subtle">Build staff records and give the right people the right FleetOS access.</p></div><div className="presence">Step {step} of 4</div></div>
      <section className="panel" style={{ marginBottom: 24 }}><div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, padding: 16 }}>{["Type", "Identity", "Rules", "Review"].map((label, i) => <div key={label} style={{ padding: "10px 12px", borderRadius: 10, background: step >= i + 1 ? "rgba(37,99,235,.10)" : "rgba(0,0,0,.04)", fontWeight: 700, fontSize: 13 }}>{i + 1}. {label}</div>)}</div></section>
