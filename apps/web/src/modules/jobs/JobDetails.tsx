...
const fieldStatuses=["TRAVELLING","ON_SITE","PAUSED"];
...
export function JobDetails({id,onClose,onChanged,fieldMode=false}:{id:string;onClose:()=>void;onChanged:()=>Promise<void>;fieldMode?:boolean}){
  ...
  async function load(){
    ...
    try{
      ...
      setJob(next);setResponses(next.worksheetResponses??{});setRiskSafe(Boolean(next.riskAssessment?.safeToProceed));setSignature(String(next.customerSignature?.name??""));setReportEmail(current=>current||next.reportEmailTo||next.contactEmail||"");
  ...
  async function saveWorksheet(){
    if(!job)return false;
    if(job.riskAssessmentRequired&&!riskSafe){setMessage("Confirm it is safe to proceed before completing the job.");return false;}
    if(job.customerSignatureRequired&&!signature.trim()){setMessage("Capture the customer name/signature before completing the job.");return false;}
  ...
  return <section className="job-detail">
    ...
    <div className="job-detail-hero panel" style={{"--job-colour":job.colour} as React.CSSProperties}><div><p className="eyebrow">{job.jobTypeName}</p><h1>{job.reference} · {job.title}</h1><p>{job.description||"No job description entered."}</p></div><div><span className={`job-priority ${job.priority.toLowerCase()}`}>{job.priority}</span><select value={job.status} disabled={busy||(!fieldMode&&!job.canManage)} onChange={e=>void updateStatus(e.target.value)}>{statuses.map(value=><option key={value}>{label(value)}</option>)}</select></div></div>
    ...
    {fieldMode&&<>
      <section className="panel job-progress-panel"><div><p className="eyebrow">Today</p><h2>Do the job</h2><p>Update your position only when it changes. Everything else is captured in the report below.</p></div><div className="job-progress-actions"><button disabled={busy||completed||job.status==="TRAVELLING"} onClick={()=>void updateStatus("TRAVELLING")}>Travelling</button><button disabled={busy||completed||job.status==="ON_SITE"} onClick={()=>void updateStatus("ON_SITE")}>On site</button><button disabled={busy||completed||job.status==="PAUSED"} onClick={()=>void updateStatus("PAUSED")}>Pause</button></div></section>
      <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Job report</p><h2>Record what happened</h2><p>Only the fields required for this job type are shown.</p></div></div><div className="form-grid">{job.worksheetSchema.map(field=><Field key={field.key} field={field} value={responses[field.key]} onChange={value=>setResponses(current=>({...current,[field.key]:value}))}/>)}</div>{job.riskAssessmentRequired&&<label className="field-declaration"><input type="checkbox" checked={riskSafe} onChange={e=>setRiskSafe(e.target.checked)}/><span>I have completed the point-of-work check and it is safe to proceed.</span></label>}{job.customerSignatureRequired&&<label>Customer name / signature *<input value={signature} onChange={e=>setSignature(e.target.value)} placeholder="Name of person accepting the work"/></label>}<div className="wizard-actions"><button className="secondary-button" disabled={busy} onClick={()=>void saveWorksheet()}>Save report</button></div></section>
      ...
      <section className="panel"><div className="panel-heading"><div><h2>Complete job</h2><p>One tap sends the completed report back to the office.</p></div></div><label>Completion note (optional)<textarea rows={3} value={note} onChange={e=>setNote(e.target.value)} placeholder="Anything the office needs to know"/></label><button className="primary-button" disabled={busy||completed} onClick={()=>void completeJob()}><CheckCircle2/> {completed?"Completed":"Complete & send to office"}</button></section>
    </>}

    {!fieldMode&&<>
      <section className="panel office-completion-panel"><div><p className="eyebrow">Customer report</p><h2>{reportReady?"Driver report ready":"Job in progress"}</h2><p>{reportReady?"Review the returned job record, approve it, then create or email the customer PDF.":"The assigned staff member sees this job automatically. Their completed report will appear here."}</p></div><div className="job-lifecycle-stamps"><span>Driver completed: {dateTime(job.submittedByDriverAt)}</span><span>Office approved: {dateTime(job.officeApprovedAt)}</span><span>Report sent: {job.reportEmailStatus||"Not sent"}</span></div><div className="office-completion-actions">{reportReady&&!job.officeApprovedAt&&<button className="primary-button" disabled={busy} onClick={()=>void approve()}><CheckCircle2/> Approve report</button>}<button disabled={busy||!job.officeApprovedAt} onClick={()=>void downloadReport()}><Download/> PDF report</button></div>{job.officeApprovedAt&&<form className="job-report-email" onSubmit={emailReport}><label>Customer email<input type="email" value={reportEmail} onChange={e=>setReportEmail(e.target.value)} placeholder="customer@example.com"/></label><label>Email message<textarea rows={3} value={reportMessage} onChange={e=>setReportMessage(e.target.value)} placeholder="Optional message"/></label><button className="primary-button" disabled={busy}><Send/> Email report</button></form>}</section>
      <section className="panel"><div className="panel-heading"><div><h2>Completed work</h2><p>The driver's type-driven report.</p></div></div>{job.worksheetSchema.length?<div className="job-sheet-preview">{job.worksheetSchema.map(field=><span key={field.key}><strong>{field.label}</strong><small>{String(job.worksheetResponses?.[field.key]??"Not completed")}</small></span>)}</div>:<p className="subtle">No worksheet fields for this job type.</p>}{job.customerSignature?.name&&<p><strong>Customer:</strong> {String(job.customerSignature.name)}</p>}</section>
