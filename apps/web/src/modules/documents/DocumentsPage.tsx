...
export function DocumentsPage(){
  ...
  return <section className="page">
    ...
    </div><p className="subtle">Private bucket Â· maximum 20 MB Â· signed links expire after 60 seconds.</p><button className="primary-button" disabled={busy}>{busy?"Uploadingâ€¦":"Upload evidence"}</button></form>
    <section className="panel"><div className="panel-heading"><div><h2>Company evidence</h2><p>Search names, types and linked FleetOS records.</p></div></div><div style={{display:"flex",gap:10,padding:"0 16px 16px",flexWrap:"wrap"}}><div className="search" style={{flex:1,minWidth:240}}><Search size={18}/><input placeholder="Search documentsâ€¦" value={search} onChange={e=>setSearch(e.target.value)}/></div><select value={filterType} onChange={e=>setFilterType(e.target.value)}><option value="ALL">All document types</option>{types.map(v=><option key={v} value={v}>{v.replaceAll("_"," ")}</option>)}</select></div><div style={{display:"grid",gap:10,padding:16,borderTop:"1px solid #edf0f4"}}>{filtered.length===0?<div className="empty-state" style={{margin:"40px auto"}}><FileSearch size={28}/><h2>No matching documents</h2><p>Upload evidence above or change the search.</p></div>:filtered.map(doc=><article key={doc.id} style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"center",border:"1px solid #e5e7eb",borderRadius:12,padding:14}}><div><strong>{doc.name}</strong><div className="subtle">{doc.type.replaceAll("_"," ")} Â· {new Date(doc.createdAt).toLocaleDateString("en-GB")}{doc.fileSize!=null?` Â· ${(doc.fileSize/1024/1024).toFixed(2)} MB`:""}</div><div style={{display:"flex",alignItems:"center",gap:6,marginTop:5,fontSize:13,color:"#526278"}}><Link2 size={14}/>{linkedLabel(doc,optionLookup)}</div></div><div style={{display:"flex",gap:8}}><button type="button" onClick={()=>void openDoc(doc)}>Open</button><button type="button" disabled={busy} onClick={()=>void removeDoc(doc)}>Remove</button></div></article>)}</div></section>
  </section>;
}
