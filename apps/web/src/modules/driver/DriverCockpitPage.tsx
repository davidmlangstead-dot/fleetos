  ...
  return <div className="driver-stack">
    ...
    <form className="driver-stack" onSubmit={submit}>
      <section className="panel driver-form-card"><h2>1. Vehicle and location</h2><div className="form-grid">
        <label>Vehicle *<select value={vehicleId} onChange={event => void loadTemplate(event.target.value, "")}><option value="">Choose vehicleâ€¦</option>{summary.vehicles.filter(vehicle => vehicle.type !== "TRAILER").map(vehicle => <option key={vehicle.id} value={vehicle.id}>{vehicle.registration} Â· {vehicle.type}</option>)}</select></label>
