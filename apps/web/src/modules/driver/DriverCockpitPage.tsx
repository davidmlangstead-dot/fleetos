...
type Job = { id: string; reference: string | null; customerName: string; collectionAddress: string | null; deliveryAddress: string | null; scheduledAt: string | null; status: string; instructions: string | null; vehicle: { id: string; registration: string } | null };
...
export function DriverCockpitPage() {
  ...
  async function moveJob(id: string, status: "IN_PROGRESS" | "DELIVERED") { setBusy(true); try { await api(`/driver-operations/jobs/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) }); await load(); } catch (jobError) { setError(jobError instanceof Error ? jobError.message : "Could not update the job."); } finally { setBusy(false); } }
