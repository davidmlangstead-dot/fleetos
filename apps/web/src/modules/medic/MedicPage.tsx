...
import { Activity, CheckCircle2, RefreshCw, ShieldAlert, Stethoscope, TriangleAlert } from "lucide-react";
...
export function MedicPage() {
  ...
  async function load() {
    ...
    try { setStatus(await api<Status>("/medic/status")); }
  ...
  async function safeClientReset() {
    ...
    try {
      await api("/medic/events", { method: "POST", body: JSON.stringify({
        ...
        recovery: "Cleared FleetOS static caches and stale workspace selection; authentication credentials were not intentionally removed.",
        recovered: true,
      }) });
  ...
  return <section className="page">
    ...
    <section className="panel" style={{marginBottom:18}}><div className="panel-heading"><div><h2>Safe recovery</h2><p className="subtle">These actions are deliberately narrow. They do not change database records, roles, RLS, secrets or deployments.</p></div></div>
