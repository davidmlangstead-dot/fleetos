...
import { Bell, ClipboardList, Gauge, Menu, MessageCircle, ShieldCheck, Truck, Users, Wrench, UserRound, Plus } from "lucide-react";
import { NavLink, Outlet } from "react-router-dom";
import { api, ACTIVE_WORKSPACE_KEY } from "../lib/api";

const nav = [
  ["/", "Today", Gauge], ["/jobs", "Jobs", ClipboardList], ["/vehicles", "Vehicles", Truck],
  ["/drivers", "Drivers", Users], ["/personal", "Personal", UserRound], ["/workshop", "Workshop", Wrench],
  ["/compliance", "Compliance", ShieldCheck], ["/messages", "Messages", MessageCircle],
] as const;
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div style={{ padding: "0 12px 14px" }}>
        <select aria-label="Company workspace" value={company?.id ?? ""} onChange={(e) => switchWorkspace(e.target.value)} style={{ width: "100%", padding: 10, borderRadius: 8 }}>
          {workspaces.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
        </select>
        <button onClick={() => void addWorkspace()} style={{ width: "100%", marginTop: 8 }}><Plus size={15} /> Add company</button>
      </div>
    ...
    <main><header className="topbar"><button className="mobile-menu" aria-label="Open navigation"><Menu /></button><div className="presence">{companyName}</div><div className="top-actions"><button className="icon-button" aria-label="Notifications"><Bell size={20} /></button><button className="avatar" aria-label="Account">{initials || "FO"}</button></div></header><Outlet /></main>
  </div>;
}
