...
commercialRouter.patch("/", requirePlatformOwner, asyncHandler(async (req, res) => {
  ...
  if (activatingPaid && commitmentStartedAt) {
    commitmentEndsAt = new Date(commitmentStartedAt);
    commitmentEndsAt.setUTCMonth(commitmentEndsAt.getUTCMonth() + commitmentMonths);
  }
  ...
  await writeAuditEvent({ companyId, actorUserId: req.user!.id, actorEmail: req.user!.email, action: "UPDATE", entityType: "COMMERCIAL_CONTROL", entityId: companyId, summary: `Updated commercial controls to ${subscriptionStatus}; vehicle limit ${vehicleLimit}; commitment ${commitmentMonths} months` });
