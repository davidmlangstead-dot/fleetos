...
operationsRouter.get("/defects", asyncHandler(async (req, res) => {
  const defects = await prisma.defect.findMany({
    ...
    include: { vehicle: { select: { id: true, registration: true } }, reportedBy: { select: { id: true, firstName: true, lastName: true } } },
...
operationsRouter.patch("/defects/:id", asyncHandler(async (req, res) => {
  ...
  const defect = await prisma.defect.update({
    ...
    data: { status, resolutionNotes: resolutionNotes || null, resolvedAt: status === "RESOLVED" ? new Date() : null },
  });
  res.json(defect);
}));
...
operationsRouter.get("/driver-hours/me", asyncHandler(async (req, res) => {
  ...
  const rows = await prisma.$queryRaw<ActivityRow[]>`
    SELECT id, "driverId", activity, "startedAt", "endedAt"
    FROM "DriverActivity"
    WHERE "companyId" = ${req.user!.companyId} AND "driverId" = ${driver.id} AND "startedAt" >= ${dayStart}
    ORDER BY "startedAt" ASC
  `;
  res.json({ driver, current: rows.findLast((x) => !x.endedAt) ?? null, totals: summarize(rows), activities: rows });
}));
...
operationsRouter.post("/driver-hours/me", asyncHandler(async (req, res) => {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.$executeRaw`
      UPDATE "DriverActivity" SET "endedAt" = ${now}, "updatedAt" = ${now}
      WHERE "companyId" = ${req.user!.companyId} AND "driverId" = ${driver.id} AND "endedAt" IS NULL
    `;
    await tx.$executeRaw`
      INSERT INTO "DriverActivity" (id, "companyId", "driverId", activity, "startedAt", "createdAt", "updatedAt")
      VALUES (${randomUUID()}, ${req.user!.companyId}, ${driver.id}, ${activity}, ${now}, ${now}, ${now})
    `;
  });
...
operationsRouter.get("/driver-hours", asyncHandler(async (req, res) => {
  ...
  const rows = await prisma.$queryRaw<ActivityRow[]>`
    SELECT id, "driverId", activity, "startedAt", "endedAt"
    FROM "DriverActivity"
    WHERE "companyId" = ${req.user!.companyId} AND "startedAt" >= ${dayStart}
    ORDER BY "startedAt" ASC
  `;
  const board = drivers.map((driver) => {
    const own = rows.filter((r) => r.driverId === driver.id);
    return { ...driver, current: own.findLast((x) => !x.endedAt) ?? null, totals: summarize(own) };
  });
  res.json(board);
}));
...
operationsRouter.get("/guardian", asyncHandler(async (req, res) => {
  ...
  const [vehicles, drivers, items, openDefects] = await Promise.all([
    prisma.vehicle.findMany({ where: { companyId: req.user!.companyId, status: "ACTIVE" }, select: { id: true, registration: true, motDue: true, taxDue: true, insuranceDue: true, tachoCalibrationDue: true } }),
    prisma.driver.findMany({ where: { companyId: req.user!.companyId, isActive: true }, select: { id: true, firstName: true, lastName: true, licenceExpiry: true, cpcExpiry: true, tachoCardExpiry: true, medicalDue: true } }),
  ...
  const add = (kind: string, label: string, date: Date | null) => {
    if (!date || date > soon) return;
    alerts.push({ severity: date < now ? "OVERDUE" : "DUE_SOON", kind, label, dueDate: date.toISOString() });
  };
  for (const v of vehicles) {
    add("MOT", `${v.registration} MOT / test`, v.motDue); add("INSURANCE", `${v.registration} insurance`, v.insuranceDue); add("TAX", `${v.registration} tax`, v.taxDue); add("TACHO_CAL", `${v.registration} tacho calibration`, v.tachoCalibrationDue);
  }
  for (const d of drivers) {
    const name = `${d.firstName} ${d.lastName}`; add("LICENCE", `${name} licence`, d.licenceExpiry); add("CPC", `${name} CPC`, d.cpcExpiry); add("TACHO_CARD", `${name} tacho card`, d.tachoCardExpiry); add("MEDICAL", `${name} medical`, d.medicalDue);
  }
